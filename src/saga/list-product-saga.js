...
function* handleGetListProduct() {
  try {
    ...
    yield put(
      listProductAction.listProductSuccess({ data: listProductDataLocal })
    );
  } catch (error) {
    yield put(
      listProductAction.listProductFailure({ errorMess: error.message })
    );
  }
}
function* handleCreateItemProduct({ payload }) {
  ...
  const handleFindItemProduct = (DataStoreProduct) => {
    const findDataStoreCart = DataStoreProduct.find(
      (element) =>
  ...
  try {
    ...
    if (role === "admin") {
      ...
      if (dataStoreAdminCart.listProduct.length === 0) {
        ...
        yield put(
          cartAction.createItemProductSucsses({ data: dataStoreAdminCart })
        );
      } else {
      ...
      if (dataStoreSaleCart.listProduct.length === 0) {
        ...
        yield setData(saleCartData.key, dataStoreSaleCart);
        yield put(
          cartAction.createItemProductSucsses({ data: dataStoreSaleCart })
        );
      }
    }
  } catch (error) {
    yield put(
      cartAction.createItemProductFailure({ errorMess: error.message })
    );
  }
}
...
function* handleSearchListProduct({ payload: textSearch }) {
  ...
  try {
    ...
    if (result) {
      yield put(
        listProductAction.searchListProductSuccess({
          data: result,
        })
      );
    } else {
