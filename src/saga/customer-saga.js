...
function* handleGetListCustomer() {
  try {
    ...
    yield put(
      customerAction.listCustomerSuccess({ data: listCustomerDataLocal })
    );
  } catch (error) {
...
function* handleSearchListCustomer({ payload: textSearch }) {
  ...
  try {
    ...
    for (let i = 0; i < listCustomerDataLocal.length; i++) {
      if (
        handleCheckString(listCustomerDataLocal[i].phoneNumber) ||
        handleCheckString(listCustomerDataLocal[i].fullName)
      ) {
    ...
    yield put(
      customerAction.searchListCustomerFailure({ errorMess: error.message })
    );
  }
}
