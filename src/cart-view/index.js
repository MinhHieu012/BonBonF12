...
export default function CartScreen() {
  ...
  const deleteCartCurrentData = (productIndex) => {
    if (isDeleteAll) {
      ...
      const filterProductData = listLocalProduct.filter(
        (_item, index) => productIndex !== index
      );
  ...
  const matchTotalPrice = () => {
    if (listCartProduct?.length) {
      const totalPrice = listCartProduct.reduce((sum, item) => {
        ...
        if (item.isSalePrice) {
          sumPriceProduct =
            (item.floorPrice + item.salePrice) * item.currentQuantity;
        } else {
          sumPriceProduct =
            (item.floorPrice - item.salePrice) * item.currentQuantity;
        }
  ...
  const getListCodeProductCart = () => {
    ...
    let removeDuplicateCode = arrCode.filter(
      (value, index) => arrCode.indexOf(value) === index
    );
  ...
  const checkValidateQuantity = () => {
    ...
    arrCodeProduct.forEach((codeProduct) => {
      ...
      listCartProduct.forEach((item, index) => {
        if (item.codeProduct === codeProduct) {
          listCartProduct[index].isValidateMinQuantity =
            listCartProduct[index].currentQuantity === 0;
          listCartProduct[index].isValidateMaxQuantity =
  ...
  const checkValidateSalePrice = () => {
    ...
    listCartProduct.forEach((item, index) => {
      if (!item.isChange) {
        listCartProduct[index].isValidateSalePrice =
          item.salePrice > item.floorPrice * 0.1;
        ...
        <ToastNotificationCommon
          ...
          Description={
            isDeleteAll
              ...
              : `Đã xóa sản phẩm ${productDelete.productName}`
          }
        />
      )}
      <SwipeList
