...
export default function ProductCard({
  ...
  const [isShowValidateMaxQuantity, setIsShowValidateMaxQuantity] =
    useState(false);
  const [isShowValidateMinQuantity, setIsShowValidateMinQuantity] =
    useState(false);
  ...
  useMemo(() => {
    ...
    if (productUpdate.isSalePrice) {
      newSumPrice =
        (data.floorPrice + productUpdate.salePrice) * productUpdate.quantity;
    } else {
      newSumPrice =
        (data.floorPrice - productUpdate.salePrice) * productUpdate.quantity;
    }
  ...
  const showValidateSalePrice = useMemo(
    () => isShowValidateSalePrice && !data.isChange,
    [isShowValidateSalePrice, data.isChange]
  );
  const showValidateMaxQuantity = useMemo(
    () => isShowValidateMaxQuantity && !data.isChange,
    [isShowValidateMaxQuantity, data.isChange]
  );
  const showValidateMinQuantity = useMemo(
    () => isShowValidateMinQuantity && !data.isChange,
    [isShowValidateMinQuantity, data.isChange]
  );
  return (
    <>
        ...
        <VStack style={styles.inforProduct} justifyContent="center">
          ...
          <VStack>
            <HStack
              ...
              style={{
                marginBottom: Dimensions.get("window").height * 0.00505,
              }}
            >
              <VStack>
                <Text
            ...
            <HStack justifyContent="space-between" alignItems="center">
              <Button
                ...
                size="xs"
                isDisabled={
                  data.isChange || formatMoney(productUpdate.salePrice) === "0"
                }
              ...
              >
                <FontAwesome
                  ...
                  color={
                    productUpdate.isSalePrice ? color.darkGreen : color.plumRed
                  }
                />
              </Button>
              <Input
          ...
          <FormControlErrorText
            fontSize={10}
            style={{ display: showValidateMaxQuantity ? "block" : "none" }}
          >
            {textConst.VALIDATE_QUALITY_MAX}
          </FormControlErrorText>
          <FormControlErrorText
            fontSize={10}
            style={{ display: showValidateMinQuantity ? "block" : "none" }}
          >
            {textConst.VALIDATE_QUALITY_MIN}
          </FormControlErrorText>
          <FormControlErrorText
            fontSize={10}
            style={{ display: showValidateSalePrice ? "block" : "none" }}
          >
